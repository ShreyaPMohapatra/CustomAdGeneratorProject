...
st.set_page_config(
    page_title="My Custom Ad Generator",
    page_icon="🎨",
    layout="wide",
...
def display_and_download_image(image_url, caption):
    """Downloads, filters, and displays an image with a download button."""
    if image_url:
        ...
        if image_data:
            ...
            filtered_image = apply_image_filter(image_data, st.session_state.image_filter)
            
            # Convert filtered PIL Image to bytes for display and download
            filtered_image_bytes = io.BytesIO()
            filtered_image.save(filtered_image_bytes, format='PNG')
            
            st.image(filtered_image_bytes.getvalue(), caption=f"Filtered Image ({st.session_state.image_filter})", use_column_width=True)

            st.download_button(
                "⬇️ Download Filtered Image",
                filtered_image_bytes.getvalue(),
                f"{caption.lower().replace(' ', '_')}_filtered.png",
                "image/png"
            )
...
def main():
    st.title("My Custom Ad Generator")
        ...
        if uploaded_file:
            ...
            with col2:
                if st.session_state.edited_image:
                    display_and_download_image(st.session_state.edited_image, "Edited Image")
